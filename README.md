...
| 🗑️ **Uninstall leftover cleanup** | Expands safe cleanup for reviewed common app cache/log leftovers after uninstall, including Electron apps, IDEs, Git clients, database/API clients, Kubernetes/container desktop tools, game launchers, sync/backup clients, search tools, password managers, PDF readers, note apps, collaboration/chat tools, terminal caches, remote access/VPN tools, transfer clients, screenshot/scanner tools, image/media tools, design tools, Markdown tools, OEM support tools, creator utilities, printing utilities, peripheral utilities, and diagnostic logs |
