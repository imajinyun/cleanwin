...
AI_TOOL_DEFINITIONS: tuple[dict[str, Any], ...] = (
    ...
    {
        "name": "cleanwin_environment_index",
        "description": "Report local CleanWin host capabilities without installing tools or writing files.",
        "risk": "readonly",
        "auto_call_allowed": True,
        "requires_confirmation": False,
        "parameters": object_schema({}),
        "argv_template": ["cleanwin", "--json", "environment-index"],
    },
    {
        "name": "cleanwin_workflow_decision",
        "description": "Validate whether a requested tool fits a workflow route and has required artifacts.",
        "risk": "readonly",
        "auto_call_allowed": True,
        "requires_confirmation": False,
        "parameters": object_schema(
            {
                "route_id": string_schema("Workflow route ID from cleanwin.workflow-router.v1."),
                "requested_tool": string_schema("Optional cleanwin tool name the host wants to call."),
                "artifacts": {
                    "type": "array",
                    "items": {"type": "string"},
                    "description": "Artifact names already produced by earlier workflow steps.",
                },
            },
            required=("route_id",),
        ),
        "argv_template": ["cleanwin", "--json", "workflow-decision"],
    },
    {
        "name": "cleanwin_workflow_trace",
        "description": "Describe the auditable artifact chain expected for CleanWin AI/MCP workflows.",
        "risk": "readonly",
        "auto_call_allowed": True,
        "requires_confirmation": False,
        "parameters": object_schema({}),
        "argv_template": ["cleanwin", "--json", "workflow-trace"],
    },
    {
