...
该检查会保持测试 pytest-native，约束直接 CLI subprocess 调用收敛到共享 helper，要求 `pytest.raises` 校验错误信息，保持遗留的直接 schema、只读布尔、`safe_to_execute`、execution-disabled flag、状态字段、summary 字段和 predicate 断言预算为空，检查已迁移文件的 collection/text/predicate/field/exact helper 采用情况，同时防止 CI 和 Docker 沙箱重新引入 `unittest discover` 或绕过项目 `.venv`。
