...
| `dev-cache` | pip、npm、Yarn、pnpm、uv、NuGet、Cargo、Go、Gradle、Maven、Android Gradle、VS Code、JetBrains 缓存 | 包含 owner、rule ID、官方清理命令、cache layer 和可安全删除理由 |
| `package-cache` | WinGet、Scoop、Chocolatey、uv、Store/App Installer、MSI、Visual Studio Installer 缓存 | 面向软件包 payload/cache artifact；repair/uninstall 敏感位置保持 review-first |
| `browser-cache` | Chrome、Edge、Brave、Firefox 的 cache-only layer | 覆盖 HTTP/code/GPU/shader/service-worker/startup/crash cache layer，同时保护 cookies、密码、会话、扩展和 profile 数据库 |
...
> rationale。真实执行还必须提供 operation log、精确确认短语，以及来自同一
> plan dry-run 的 confirmation token。`app-leftovers`、registry、startup、service、scheduled task、
...
`scan-governance` 也会静态校验 wrapper 边界：artifact path 必须经过
`Resolve-ArtifactPath`，native command 必须匹配已审阅的只读 allowlist，直接写
artifact 的 API 只能出现在 artifact helper 函数内。

`windows-artifact-layout` 定义 native collector 的真实 Windows artifact
目录和 manifest contract，包含允许的 artifact 扩展名、预期顶层文件和目录、
manifest/record 必填字段、SHA-256 hash 要求、Windows version 与
managed/unmanaged 状态等上下文字段，以及禁止出现的命令片段。
`windows-artifact-validate` 会读取已有的 `manifest.json` 和本地 artifact 文件，
输出 `cleanwin.windows-native-artifact-validation.v1` issues，报告缺失字段、
不安全 relative path、不支持的扩展名、危险命令片段、summary 不一致、缺失文件
和 SHA-256 mismatch。Validator 不会运行 collector，也不会执行
PowerShell、DISM、registry、service、scheduled task、package manager、cleanup、
repair、remove、import、disable 或 uninstall 命令。
...
confirmations，输出 `cleanwin.promotion-gate-validation.v1`，但不会启用执行。

`low-risk-cache-readiness` 是面向未来 cache execution promotion 的只读治理
contract。它聚合低风险 cache candidate 进入人工执行复核前必须具备的 evidence：
dry-run token reference、operation log reference、locked state reference、
operation log readiness reference、identity check reference、sensitive
exclusions、rule quality gate、recycle mode 和 confirmation phrase。Browser
cache promotion gate 和 preset plan template 都会要求同一组 readiness evidence。
该报告保持 `execution_enabled=false`，不会运行清理。
配套的 `cleanwin.low-risk-cache-readiness-validation.v1` contract 会把同一组
evidence 校验成机器可读 blocker；workflow decision/trace contract 也会要求该
validation artifact，然后才允许进入 recycle execution handoff。
`operation-log-readiness` 是 operation log handoff 的结构化只读 companion
contract。配套的 `cleanwin.operation-log-readiness-validation.v1` validator 会要求
structured readiness ref、recycle delete mode、匹配的 dry-run token ref 和匹配的
plan fingerprint，然后 low-risk cache readiness validator 才能通过。

`contract-exposure-matrix` 会输出 `cleanwin.contract-exposure-matrix.v1` 和配套
`cleanwin.contract-exposure-validation.v1` 一致性检查，用于把治理 contract 在
schema registry、CLI command、`ai-tools --provider`、MCP resource、workflow trace
reference、evidence bundle reference 和 docs mention 之间对齐。该矩阵只读运行，
只报告 `MISSING_SCHEMA_REGISTRY_ENTRY`、`MISSING_CLI_COMMAND`、
`MISSING_AI_TOOLS_PROVIDER`、`MISSING_MCP_RESOURCE`、`MISSING_DOCS_REFERENCE`、
`MISSING_WORKFLOW_TRACE_REFERENCE` 和 `MISSING_EVIDENCE_BUNDLE_REFERENCE` 等缺口。
可选 matrix surface 会标记为 `not_applicable`，不会被计为 missing。
...
Translated candidate 还会通过 `cleanwin.external-rule-quality-summary.v1` 输出
`external-rule-quality` gate，包含 risk、recoverability、dangerous path count、
unsupported semantic count、active marker gap、sensitive exclusion gap 和 fixture
coverage gap。
...
- 暴露 `cleanwin://ai/tools`、`cleanwin://ai/host-policy`、`cleanwin://ai/schema-registry`、`cleanwin://ai/schema-validation`、`cleanwin://ai/readiness`、`cleanwin://ai/self-test`、`cleanwin://ai/runbook`、`cleanwin://ai/workflow-router`、`cleanwin://ai/environment-index`、`cleanwin://ai/workflow-decision`、`cleanwin://ai/workflow-trace`、`cleanwin://engineering/doctor`、`cleanwin://engineering/recovery-readiness`、`cleanwin://engineering/low-risk-cache-readiness`、`cleanwin://engineering/operation-log-readiness`、`cleanwin://engineering/contract-exposure-matrix`、`cleanwin://inventory/installed-apps`、`cleanwin://inventory/windows`、`cleanwin://plan/official-command-plan`、`cleanwin://inventory/debloat-privacy`、`cleanwin://inventory/startup-services` 和 `cleanwin://plan/review-sample` 等资源。
...
- `windows-evidence-bundle` 会输出 `cleanwin.windows-evidence-bundle.v1` 作为由调用方管理的 JSONL evidence chain，串联 report refs、snapshot refs、simulated plan refs、rollback drill refs、promotion gate refs、low-risk cache readiness refs、recovery readiness refs 和 Windows smoke CI refs。该命令不会写文件、采集 Windows artifact，也不会运行 cleanup、registry、AppX、service、task、DISM 或 PowerShell 命令。
