...
| `dev-cache` | pip, npm, Yarn, pnpm, uv, NuGet, Cargo, Go, Gradle, Maven, Android Gradle, VS Code, JetBrains caches | Includes owner, rule ID, official cleanup command, cache layer, and safe-to-delete rationale |
| `package-cache` | WinGet, Scoop, Chocolatey, uv, Store/App Installer, MSI, Visual Studio Installer caches | Targets package payload/cache artifacts with repair/uninstall-sensitive locations kept review-first |
| `browser-cache` | Cache-only layers for Chrome, Edge, Brave, and Firefox | Covers HTTP/code/GPU/shader/service-worker/startup/crash cache layers while protecting cookies, passwords, sessions, extensions, and profile databases |
...
`debloat-privacy-report` is report-only and now covers a 180-check Windows
...
SmartScreen, Widgets, Edge SmartScreen/search/autofill/metrics/prediction,
Defender/security-sensitive protections, sensor/location controls, Store and
Office connected-experience policies. It also classifies bundled AppX packages
for manual review without uninstalling or changing policy.
