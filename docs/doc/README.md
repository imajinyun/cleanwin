...
| `app-leftovers` | Common app cache/log leftovers after uninstall | Covers reviewed leftovers like Slack, Teams classic, Discord, Zoom, VS Code, JetBrains logs, Docker Desktop logs, Postman, Notion, Figma, OBS Studio, Spotify, Adobe Creative Cloud, Office telemetry, game launchers, GPU shader caches, Telegram, Signal, WhatsApp, Cursor, Android Studio, VirtualBox logs, VLC artwork cache, 1Password logs, GitHub Desktop, Obsidian, Unity Hub, Unreal/EA/GOG/Ubisoft launcher caches, Dropbox logs, Logitech G HUB logs, and Razer Synapse logs; skips paths when active install markers still exist, including globbed versioned install markers such as `app-*`, IDE version folders, and `*.exe` |
