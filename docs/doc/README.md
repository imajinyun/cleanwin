...
| `app-leftovers` | Common app cache/log leftovers after uninstall | Covers reviewed leftovers like Slack, Teams classic/new Teams, Discord, Zoom, Skype, Webex, VS Code, JetBrains/DataGrip logs, Docker Desktop logs, Postman, Notion, Figma, OBS Studio, Spotify, Adobe Creative Cloud, Office telemetry, game launchers, GPU shader caches, Telegram, Signal, WhatsApp, Cursor, Android Studio, VirtualBox logs, VLC artwork cache, 1Password logs, GitHub Desktop, Obsidian, Unity Hub, Unreal/EA/GOG/Ubisoft launcher caches, Dropbox logs, OneDrive/Google Drive/iCloud diagnostics, Todoist, Linear, Canva, PowerShell startup cache, Windows Terminal diagnostics, Snagit/Camtasia logs, remote access and VPN diagnostics, Wireshark/FileZilla/WinSCP state or logs, calibre cache, qBittorrent logs, Git client diagnostics, Kubernetes/container desktop diagnostics, database/API client diagnostics, image/media tool caches, design tool caches, Markdown tool renderer caches, printing utility logs, PowerToys logs, Logitech G HUB logs, Corsair iCUE logs, and Razer Synapse logs; skips paths when active install markers still exist, including globbed versioned install markers such as `app-*`, package version folders, IDE version folders, and `*.exe` |
